...
# I used "rails generate model ____" to create 4 models: 1. Studio, 2. Movie, 3. Actor, 4. Role
# After that, I inserted the code to create the columns in the db/migrate files 
...
for m in Movie.all
    ...
    rated = m["rated"]
    studio = Studio.find_by({ "id" => m["studio_id"]})["name"]
...
for r in Role.all
    ...
    movie = Movie.find_by({ "id" => r["movie_id"]})["title"]
    actor = Actor.find_by({ "id" => r["actor_id"]})["name"]
